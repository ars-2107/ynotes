...
const manifests = [
  ...
  // The Claude Code plugin manifest carries the same version, so an
  // installed plugin is matchable to a binary release (invariant 9).
  { path: '.claude-plugin/plugin.json', main: false },
...
if (check && drift) {
  console.error('\nversion manifests are stale. Run: node scripts/sync-version.mjs');
  process.exit(1);
}
console.log(check ? `all version manifests match Cargo.toml (${version})` : `done (${version})`);
