//! R4: anchor to the *syntax tree*, not the text.
//!
//! R2/R3 anchor on characters, so a wholesale reformat (rustfmt, prettier,
//! black) or a large refactor defeats them even though a human still calls it
//! "the body of `parseConfig`". R4 stores a path to the nearest **named**
//! enclosing construct plus a content fingerprint of its subtree (node kinds
//! with identifier and literal text), and relocates by *name* in the
//! re-parsed tree. Reformatting changes concrete
//! syntax but not the AST shape or the names, so the anchor survives it; a
//! move changes line numbers but not the name-qualified path, so it survives
//! that too. If the language is unknown or the file does not parse, R4
//! reports nothing and the ladder is unaffected — invariant #4 intact.
//!
//! tree-sitter's C cores live behind the grammar crates' own FFI, not ours;
