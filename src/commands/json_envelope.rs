...
//!
//! Shape (`v: 5` of the agent contract):
//!
//! - success: `{"success": true, "v": 5, "data": <command payload>}`
//! - failure: `{"success": false, "v": 5, "error": "<message>", "type": "<kind>"}`
//!
...
/// (invariant #7).
pub(crate) const CONTRACT_VERSION: u8 = 5;
...
pub(crate) fn print_error(message: &str, kind: ErrorKind) {
    ...
    let line = serde_json::to_string_pretty(&envelope).unwrap_or_else(|_| {
        r#"{"success":false,"v":5,"error":"failed to render JSON error envelope","type":"render"}"#
            .to_string()
    });
