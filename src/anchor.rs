//! The resolver: turn a saved [`SelectorBundle`] plus the current file into a
//! single answer — *is this region still here, and is it intact?*
//!
//! Each rung re-finds the region independently, but the rungs are not equal
//! and answer different questions. Only the **content rungs** — exact quote
//! (R2) and fuzzy alignment (R3) — and a **content-verified structural match**
//! (R4 at a fully-identical fingerprint) confirm the region's code is
//! present. Git transport (R1) locates a *line number* and may corroborate
//! a content rung's range, but never decides a verdict on its own — git
//! will carry a deleted range straight onto its deletion point. The saved
//! position (R5) is purely diagnostic — surfaced in `--explain` so an
//! agent can see what the user originally pointed at, but never consulted
//! by the verdict, because it is derived from the saved range (clamped when
//! the file shrank, never independently re-located), so treating its
//! overlap with any other rung as corroboration would be circular.
//!
//! The verdict is one of three honest categories —
//! [`Anchored`](AnchorStatus::Anchored) (here and intact),
//! [`Drifted`](AnchorStatus::Drifted) (found, but moved or changed), or
//! [`Orphaned`](AnchorStatus::Orphaned) (no content rung located it; surfaced
//! with its last known position, never silently dropped). There is no numeric
//! confidence score — a category an agent can branch on is the honest signal;
//! a fabricated 0–100 number is not.
...
/// is the trust signal. Each category maps to one agent action — trust it,
/// re-read it, or treat it as historical — and a 0–100 score adds none.
#[derive(Debug, Clone)]
...
fn rung_quote(bundle: &SelectorBundle, file: &SourceFile) -> RungResult {
    ...
    match matches.as_slice() {
        [] => RungResult::Miss,
        [only] => RungResult::Hit {
            range: to_range(*only, needle.len()),
            score: 95,
        },
        many => {
