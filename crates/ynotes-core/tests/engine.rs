...
#[test]
fn a_note_without_review_basis_is_seeded_from_its_bundle() {
    let (_dir, source) = nine_line_file();
    let note = note_for(
        &source,
        ynotes::Scope::Range,
        ynotes::LineRange::new(5, 5).unwrap(),
    );
    let expected = note.bundle.quote.sha256.clone();
    ...
    // Missing review state must derive from saved code, never current code.
    let migrated = serde_json::from_value::<ynotes::Note>(value)
        .expect("a record written before the review lifecycle must still load");
    assert_eq!(
        migrated.review_basis.sha256, expected,
        "the seeded basis must be the bundle's quote hash: that is the code \
         `save` verified this body against"
    );
}
