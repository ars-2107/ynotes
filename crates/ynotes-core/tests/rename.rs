...
#[test]
fn file_discovery_follows_verified_renames_without_rewriting_notes() {
    let dir = tempfile::tempdir().unwrap();
    let root = dir.path();
    assert!(init_repo(root));
    let old = root.join("old.rs");
    std::fs::write(&old, "fn target() {\n    important_work();\n}\n").unwrap();
    assert!(git(root, &["add", "old.rs"]));
    assert!(git(root, &["commit", "-qm", "initial"]));
    let store = Store::init(root).unwrap();
    let note = save_note(&store, &old, LineRange::new(1, 3).unwrap(), "why");
    assert!(git(root, &["mv", "old.rs", "new.rs"]));
    for committed in [false, true] {
        if committed {
            assert!(git(root, &["commit", "-qm", "rename"]));
        }
        let inventory = ynotes::files(&store).unwrap();
        assert_eq!(inventory.total, 1);
        assert_eq!(inventory.files.len(), 1);
        assert_eq!(inventory.files[0].target, "new.rs");
        assert_eq!(inventory.files[0].notes, 1);
        assert_eq!(store.notes_for("old.rs").unwrap().notes[0].id, note.id);
        assert!(store.notes_for("new.rs").unwrap().notes.is_empty());
    }
    std::fs::write(root.join("new.rs"), "struct Unrelated;\n").unwrap();
    let inventory = ynotes::files(&store).unwrap();
    assert_eq!(
        inventory.files[0].target, "old.rs",
        "an uncorroborated rename must retain the historical path"
    );
}
...
/// A partial clone can know a recent local rename while lacking older history.
/// `git log --follow` writes that valid rename to stdout, then exits non-zero
/// when traversal reaches the missing object. The useful prefix must still be
/// consumed; otherwise query silently misses context that reanchor can find
/// from the note's forward git selector.
#[test]
fn renamed_from_keeps_valid_history_emitted_before_a_git_failure() {
    let dir = tempfile::tempdir().unwrap();
    let root = dir.path();
    if !init_repo(root) {
        eprintln!("skipping: no usable `git` binary");
    ...
    std::fs::write(root.join("old.rs"), "fn target() {}\n").unwrap();
    git(root, &["add", "."]);
    if !git(root, &["commit", "-q", "-m", "root"]) {
        eprintln!("skipping: git commit unavailable");
        return;
    }
    let root_commit = Command::new("git")
        ...
        .output()
        .unwrap();
    let root_commit = String::from_utf8(root_commit.stdout).unwrap();
    let root_commit = root_commit.trim();

    std::fs::write(root.join("history.txt"), "local intermediate commit\n").unwrap();
    git(root, &["add", "."]);
    git(root, &["commit", "-q", "-m", "intermediate"]);
    git(root, &["mv", "old.rs", "new.rs"]);
    git(root, &["commit", "-q", "-m", "rename"]);

    // Remove only the oldest commit object. HEAD, its parent, and both sides
    // of the recent rename remain local, matching an offline partial clone
    // whose traversal runs out of promised history after emitting the rename.
    let object = root
        .join(".git/objects")
        .join(&root_commit[..2])
        .join(&root_commit[2..]);
    std::fs::remove_file(&object).expect("root commit is a loose test object");
    ...
    let raw = Command::new("git")
        ...
        .output()
        .unwrap();
    assert!(!raw.status.success(), "fixture must end in a git failure");
    assert!(
        String::from_utf8_lossy(&raw.stdout).contains("old.rs"),
        "git must emit the recent rename before hitting missing history"
    );

    let ctx = GitContext::discover(root).expect("repo discovered");
    assert_eq!(ctx.renamed_from("new.rs"), vec!["old.rs"]);
}

/// The two sides of a pending migration always travel together, on a per-file
/// query as much as on an inventory read.
///
/// `relocated_to` restates the queried path here, so it is easy to treat as
/// inventory-only and drop from this path. That would break the rule the docs
/// give a consumer, a note carrying `relocated_from` carries `relocated_to`
/// too, and force an agent to branch on which call produced the note.
#[test]
fn a_file_query_reports_both_sides_of_a_pending_rename() {
    let dir = tempfile::tempdir().unwrap();
    let root = dir.path();
    if !init_repo(root) {
        eprintln!("skipping: no usable `git` binary");
    ...
    git(root, &["mv", "old.rs", "new.rs"]);
    let found = query(&store, &root.join("new.rs"), LineSpec::Whole).unwrap();
    assert_eq!(
        found.matched.len(),
        1,
        "the note must surface at the new path"
    );
    assert_eq!(found.matched[0].relocated_from.as_deref(), Some("old.rs"));
    assert_eq!(
        found.matched[0].relocated_to.as_deref(),
        Some("new.rs"),
        "a file query must name the destination too, not only the inventory"
    );
}

#[test]
fn list_resolves_staged_and_committed_renames_without_persisting_them() {
    let dir = tempfile::tempdir().unwrap();
    let root = dir.path();
    if !init_repo(root) {
        eprintln!("skipping: no usable `git` binary");
    ...
    git(root, &["mv", "old.rs", "new.rs"]);
    let staged = list(&store, None).unwrap();
    assert_eq!(staged.notes.len(), 1);
    assert_eq!(staged.notes[0].note.id, original.id);
    assert_eq!(staged.notes[0].relocated_from.as_deref(), Some("old.rs"));
    assert_eq!(staged.notes[0].relocated_to.as_deref(), Some("new.rs"));
    assert!(matches!(
        staged.notes[0].resolution.status,
        ynotes::AnchorStatus::Anchored
    ));

    std::fs::write(root.join("new.rs"), "fn target() {\n    work_v2();\n}\n").unwrap();
    git(root, &["add", "."]);
    git(root, &["commit", "-q", "-m", "rename and edit"]);
    let committed = list(&store, None).unwrap();
    assert_eq!(committed.notes.len(), 1);
    assert_eq!(committed.notes[0].note.id, original.id);
    assert_eq!(committed.notes[0].relocated_from.as_deref(), Some("old.rs"));
    assert_eq!(committed.notes[0].relocated_to.as_deref(), Some("new.rs"));
    assert!(matches!(
        committed.notes[0].resolution.status,
        ynotes::AnchorStatus::Drifted { .. }
    ));

    assert_eq!(
        store.notes_for("old.rs").unwrap().notes.len(),
        1,
        "list is read-only and must not persist the relocation"
    );
    assert!(store.notes_for("new.rs").unwrap().notes.is_empty());
}
...
fn reanchor_re_targets_a_note_after_a_committed_rename() {
    ...
    // Invariant #11: relocation moves a note, it does not re-review it. The
    // region's text is identical at the new path, so the body is still checked
    // against the code it points at and no review is outstanding. Flagging one
    // here would train a reader to ignore the flag on the renames where the
    // code really did change.
    assert!(
        !reloc.review_required,
        "a pure rename leaves nothing to re-review"
    );
...
/// The complement of the pure rename: the file moved *and* the noted region was
/// edited in the same commit. Location and trust are separate (invariant #11),
/// so relocation must carry the note to the new path while still reporting that
/// its body has not been checked against the code now there. Reporting the move
/// without the flag would quietly upgrade an unreviewed note to `anchored`.
#[test]
fn a_rename_that_also_edits_the_region_still_demands_a_review() {
    let dir = tempfile::tempdir().unwrap();
    let root = dir.path();
    if !init_repo(root) {
        eprintln!("skipping: no usable `git` binary");
    ...
    let store = Store::init(root).unwrap();
    let original = save_note(&store, &old, LineRange::new(2, 4).unwrap(), "hot path");
    assert!(original.bundle.git.is_some(), "needs a git selector");

    // Rename and edit the region in one commit. The region is still locatable
    // at the new path, so this relocates rather than orphans.
    git(root, &["mv", "old.rs", "new.rs"]);
    std::fs::write(
        root.join("new.rs"),
        "fn one() {}\nfn target() {\n    work_v2();\n}\nfn z() {}\n",
    ...
    assert_eq!(
        report.relocated.len(),
        1,
        "the note follows the rename: {report:?}"
    );
    assert!(
        report.relocated[0].review_required,
        "the code at the new path differs from the reviewed basis, so the \
         relocation must not pass the note off as reviewed"
    );

    // And the read side agrees: still stale until someone confirms it.
    let found = query(&store, &root.join("new.rs"), LineSpec::Whole).unwrap();
    assert_eq!(found.matched.len(), 1);
    assert!(
        matches!(
            found.matched[0].resolution.status,
            ynotes::AnchorStatus::Drifted { .. }
        ),
        "an edited-through-rename note stays drifted: {:?}",
        found.matched[0].resolution.status
    );
}
