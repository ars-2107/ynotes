//! In-memory source lines and inclusive line ranges for selector matching.
//!
//! Read each target once into a [`SourceFile`] so content matchers share the
//! same text snapshot. Git history access is handled by [`crate::git`].
