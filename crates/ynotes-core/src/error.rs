//! Typed engine errors shared by all front-ends.
//!
//! Callers match [`Error`] variants to distinguish usage, storage, and resolution
//! failures. The CLI adds user-facing diagnostics and process exit codes.
...
pub enum Error {
    ...
    /// A note cannot be explicitly confirmed because its region is no longer
    /// locatable. Confirmation advances trust, so it must never fall back to
    /// a saved line number or guess at replacement code.
    #[error("note `{id}` is orphaned; re-place or update it before confirming its context")]
    Unconfirmable {
        /// The full id of the note whose region could not be located.
        id: String,
    },

    /// A note cannot be explicitly confirmed because only a similarity rung
    /// located its region. Confirmation advances trust, so the region must be
    /// *identified*, by git transport, an exact quote, or the structural path
    ///, and never merely resembled.
    ///
    /// The remedy is the documented order of operations rather than a dead end:
    /// `reanchor` refreshes the selectors against current code first, which is
    /// what makes the following `confirm` exact.
    #[error(
        "note `{id}` resolved by similarity, not identity; run `ynotes reanchor` first, then confirm"
    )]
    Unidentified {
        /// The full id of the note whose region could not be identified.
        id: String,
    },
