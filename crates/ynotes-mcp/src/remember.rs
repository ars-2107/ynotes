//! Save or supersede a note at an explicitly selected root.
//! Auto-creation requires a Git root to avoid creating an undiscoverable nested store.
...
use crate::tools::{explicit_root, file_for_root, line_spec, ok_json, tool_err};
...
/// none exists it bootstraps one at the explicitly selected root, provided
/// that root is a repository root, reporting `store_created`. A root deeper in
/// the tree is refused as a usage error rather than silently given its own
/// store. Neither store nor file resolution consults the server cwd.
pub(crate) fn run(args: &RememberArgs) -> CallToolResult {
    ...
    // Deliberate trade: the store is bootstrapped before the remember request
    // is fully validated, so a usage-failed first save can leave a fresh empty
    // `.ynotes` behind. Validating first would need discovery split from
    // init (`relativize` and the symlink guard need the store root); an
    // empty store in a repo mid-adoption is harmless, so the simple order
    // stands.
    let (store, store_created) = match Store::open_or_init_at_root(&root) {
        Ok(pair) => pair,
        // A `root` that is not a repository root is a caller mistake, not a
        // site to conjure a store at: the note would land in a second store
        // that later reads at the true root never see.
        //
        // The engine's own message is CLI-shaped ("no store found from X
        // *upward*, run `ynotes init`") and would mislead here twice over: no
        // upward search happened (invariant #13 forbids it), and the caller is
        // an agent that may not be able to run a shell at that path. Say what
        // this face actually requires instead.
        Err(ynotes::Error::StoreNotFound { .. }) => {
            return tool_err(
                "usage",
                format!(
                    "`{}` is not a repository root, so there is nowhere to put a \
                     note store: pass the repository root (the directory holding \
                     `.git`), or create a store there first",
                    root.display()
                ),
            );
        }
    ...
    let outcome = match store.save_superseding(&note) {
        Ok(outcome) => outcome,
        Err(e) => return tool_err("engine", e),
    };
    ok_json(&SaveData {
        ...
        store_created,
        neighbours: ynotes::contract::NeighbourView::project(&outcome.neighbours),
    })
}
...
mod tests {
    ...
    /// A tempdir with a `.git/` directory (so the bootstrap accepts it as a
    /// repository root) and a `f.txt` of three lines, but no store yet.
    ...
    #[test]
    fn remember_without_a_repository_root_errors_naming_the_requirement() {
        let dir = tempfile::tempdir().unwrap();
        let file = dir.path().join("f.txt");
        std::fs::write(&file, "alpha\n").unwrap();
        ...
        assert_eq!(
            result.is_error,
            Some(true),
            "expected a refusal: {result:?}"
        );
        let message = text_of(&result);
        // Not the engine's CLI-shaped "no store found from X upward (run
        // `ynotes init`)": no upward search happened (invariant #13 forbids
        // one), and the caller is an agent that may have no shell at that path.
        // The refusal has to name what this face actually needs, a repository
        // root, or the agent retries the same wrong root.
        assert!(
            message.contains("not a repository root"),
            "the refusal must name the real requirement: {message}"
        );
        assert!(
            !message.contains("upward"),
            "no upward search happens here; claiming one would be a lie: {message}"
        );
        assert!(
            !dir.path().join(".ynotes").exists(),
            "a directory that is not a repository root must not grow a store"
        );
    }

    /// The misplacement this guard exists for: an agent working inside a crate
    /// naturally passes that crate's directory as `root`. Creating a store
    /// there files the note where every later read at the repository root
    /// reports it missing, context lost with nothing raised (invariant #4).
    #[test]
    fn remember_refuses_to_create_a_second_store_below_the_repository_root() {
        let (dir, _file) = git_repo_no_store();
        let nested = dir.path().join("crates").join("inner");
        std::fs::create_dir_all(&nested).unwrap();
        let file = nested.join("g.txt");
        std::fs::write(&file, "alpha\nbravo\n").unwrap();

        let result = run(&args_for(&file, Some(1), Some(1), "why alpha matters"));
        assert_eq!(
            result.is_error,
            Some(true),
            "expected a refusal: {result:?}"
        ...
        assert!(
            !nested.join(".ynotes").exists(),
            "no second store may appear below the repository root"
        );
    }
