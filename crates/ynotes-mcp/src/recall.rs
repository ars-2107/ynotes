...
use crate::tools::{discover_for, line_spec, ok_json, store_absent, tool_err};
...
/// writing), and serialises the v12 `queryData`/`queryCountData` payload. A repo
...
pub(crate) fn run(args: &RecallArgs) -> CallToolResult {
    ...
    // Discover from the file's own directory (absolute where possible) so the
    // right store is found regardless of the server's cwd.
    let anchor = std::path::absolute(&file).unwrap_or_else(|_| file.clone());
    let search_from = anchor.parent().unwrap_or_else(|| Path::new("."));
    let store = match discover_for(search_from) {
        Ok(Some(s)) => s,
