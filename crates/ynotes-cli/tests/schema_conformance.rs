//! Validate every command's real JSON output against the published schema.
...
/// Create a store for cases whose subject is another command.
fn init(dir: &Path) {
...
fn every_command_json_payload_conforms_to_the_schema() {
    ...
    assert_conforms(&v, "init", &run_json(p, &["init", "--json"]));
    ...
    // files: the cheap orientation read, annotated files, nothing resolved.
    assert_conforms(&v, "files", &run_json(p, &["files", "--json"]));
    // show <id> (v11): the by-id read, resolved against current code.
    ...
    assert_conforms(&v, "confirm", &run_json(p, &["confirm", &id, "--json"]));
    assert_conforms(
        &v,
        "reanchor (dry-run)",
...
#[test]
fn files_store_absent_payload_conforms_to_the_schema() {
    let dir = tempfile::tempdir().unwrap();
    let result = run_json(dir.path(), &["files", "--json"]);
    assert_eq!(result["success"], true);
    assert_conforms(&schema(), "files (store absent)", &result);
}
...
fn the_validator_rejects_non_conforming_output() {
    ...
    let unknown_payload = serde_json::json!({
        "success": true, "v": 1, "data": { "not_a_real_payload": 1 }
    });
    assert!(
