...
#[test]
fn files_without_a_store_succeeds_without_creating_one() {
    ...
    assert_eq!(json["data"], serde_json::json!({"store": "absent"}));
    ynotes()
        .current_dir(dir.path())
        .arg("files")
        .assert()
        .success();
    assert!(!dir.path().join(".ynotes").exists());
}
...
/// A malformed index is rebuildable cache state, so doctor keeps the store
/// readable and reports the cache status under health.
#[test]
fn doctor_json_reports_a_malformed_index_as_health() {
    let dir = tempfile::tempdir().expect("tempdir");
    ynotes()
        .current_dir(dir.path())
        .arg("init")
    ...
    assert!(
        v["data"]["store"]["found"]["path"]
            .as_str()
            .unwrap()
            .ends_with(".ynotes")
    );
    assert_eq!(v["data"]["health"]["index"]["status"], "malformed");
}
...
#[test]
fn init_json_reports_creation_and_idempotent_reopen() {
    let dir = tempfile::tempdir().expect("tempdir");
    let first = ynotes()
        .current_dir(dir.path())
        .args(["init", "--json"])
    ...
    let first: serde_json::Value =
        serde_json::from_slice(&first.get_output().stdout).expect("valid JSON");
    assert_eq!(first["success"], true);
    assert_eq!(first["data"]["created"], true);
    assert!(first["data"]["root"].as_str().unwrap().ends_with(".ynotes"));
    ...
    let second: serde_json::Value =
        serde_json::from_slice(&second.get_output().stdout).expect("valid JSON");
    assert_eq!(second["data"]["created"], false);
}

#[test]
fn init_json_failure_uses_the_shared_envelope() {
    let dir = tempfile::tempdir().expect("tempdir");
    std::fs::create_dir(dir.path().join(".ynotes")).unwrap();
    let out = ynotes()
        .current_dir(dir.path())
        .args(["init", "--json"])
        .assert()
    ...
    let value: serde_json::Value =
        serde_json::from_slice(&out.get_output().stdout).expect("valid JSON envelope");
...
/// root and reports it. A subsequent save does not repeat the creation message.
#[test]
...
#[test]
fn reanchor_keeps_edited_context_stale_until_confirm() {
    let dir = tempfile::tempdir().expect("tempdir");
    ynotes()
        .current_dir(dir.path())
        .arg("init")
    ...
    std::fs::write(&file, "fn target() {\n    work_async();\n}\n").unwrap();
    let maintenance = ynotes()
        .current_dir(dir.path())
        .args(["reanchor", "--json"])
        .assert()
        .success();
    let maintenance: serde_json::Value =
        serde_json::from_slice(&maintenance.get_output().stdout).unwrap();
    assert_eq!(
        maintenance["data"]["changed"][0]["review_required"], true,
        "the maintenance result must flag the stale note directly"
    );

    let stale = ynotes()
        .current_dir(dir.path())
        .args(["query", "x.rs", "--json"])
        .assert()
        .success();
    let stale: serde_json::Value = serde_json::from_slice(&stale.get_output().stdout).unwrap();
    assert_eq!(stale["data"]["notes"][0]["status"], "drifted");
    assert_eq!(stale["data"]["notes"][0]["stale"], true);
    let id = stale["data"]["notes"][0]["id"].as_str().unwrap().to_owned();

    let acknowledged = ynotes()
        .current_dir(dir.path())
        .args(["confirm", &id, "--json"])
        .assert()
        .success();
    let acknowledged: serde_json::Value =
        serde_json::from_slice(&acknowledged.get_output().stdout).unwrap();
    assert_eq!(acknowledged["data"]["previous_id"], id);
    assert_eq!(acknowledged["data"]["changed"], true);
    ...
    let current = ynotes()
        ...
        .assert()
        .success();
    let current: serde_json::Value = serde_json::from_slice(&current.get_output().stdout).unwrap();
    assert_eq!(current["data"]["notes"][0]["status"], "anchored");
    assert_eq!(current["data"]["notes"][0]["stale"], false);
}

/// The text remedy must name a usable id: `reanchor` is the only command that
/// learns which notes need confirming, so a placeholder forces the user to
/// correlate `list --json` by target and range before they can act.
#[test]
fn reanchor_text_names_the_id_to_confirm() {
    let dir = tempfile::tempdir().expect("tempdir");
    ynotes()
        .current_dir(dir.path())
        .arg("init")
    ...
    let preview = ynotes()
        ...
        .assert()
        .success();
    let preview = String::from_utf8(preview.get_output().stdout.clone()).unwrap();
    assert!(
        preview.contains("review required"),
        "a dry run still flags the pending review: {preview}"
    );
    assert!(
        !preview.contains("ynotes confirm"),
        "a dry run writes nothing, so it must not offer an id: {preview}"
    );
    ...
    let applied = ynotes()
        ...
        .assert()
        .success();
    let applied = String::from_utf8(applied.get_output().stdout.clone()).unwrap();
    let remedy = applied
        .lines()
        .find(|line| line.contains("ynotes confirm"))
        .unwrap_or_else(|| panic!("reanchor must name the id to confirm: {applied}"));
    let id = remedy
        .split("ynotes confirm ")
        .nth(1)
        .and_then(|rest| rest.split_whitespace().next())
        .map(|token| token.trim_end_matches('`'))
        .expect("an id follows the command");
    assert!(
        !id.is_empty() && id.chars().all(|c| c.is_ascii_hexdigit()),
        "the remedy must carry a real hex id, not a placeholder: {remedy}"
    );

    // The printed id has to be the one the store actually holds, not merely
    // hex-shaped.
    ynotes()
        .current_dir(dir.path())
        .args(["confirm", id])
...
#[test]
fn prune_keeps_a_note_whose_file_was_renamed_and_edited() {
    let dir = tempfile::tempdir().expect("tempdir");
    let git = |args: &[&str]| {
    ...
    std::fs::write(dir.path().join("old.rs"), "fn target() {\n    work();\n}\n").unwrap();
    git(&["add", "."]);
    git(&["commit", "-q", "-m", "init"]);
    ...
    ynotes()
        .current_dir(dir.path())
        .args(["save", "old.rs", "1:3", "-m", "keep across rename"])
        .assert()
        .success();
    ...
    let listed = ynotes()
        ...
        .assert()
        .success();
    let listed: serde_json::Value = serde_json::from_slice(&listed.get_output().stdout).unwrap();
    assert_eq!(listed["data"]["notes"][0]["target"], "old.rs");
    assert_eq!(listed["data"]["notes"][0]["relocated_from"], "old.rs");
    assert_eq!(listed["data"]["notes"][0]["relocated_to"], "new.rs");
    assert_eq!(listed["data"]["notes"][0]["status"], "drifted");
    ...
    for args in [
        ...
        assert!(
            value["data"]["pruned"].as_array().unwrap().is_empty(),
            "a located rename is not an orphan: {value}"
        );
        assert_eq!(value["data"]["kept"], 1);
    }
    ...
    ynotes()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("keep across rename"));
}
...
fn reindex_rebuilds_a_deleted_index() {
    ...
    // Reads recover from the surviving record and flag the missing cache. They
    // do not rebuild it implicitly.
    ynotes()
        .current_dir(dir.path())
        .args(["query", "code.rs", "1"])
        .assert()
        .success()
        .stdout(predicate::str::contains("load-bearing"))
...
fn reindex_dry_run_writes_nothing() {
    ...
    // The dry run wrote nothing, so the index is still missing. The read still
    // recovers the note and reports the repair path.
    ynotes()
        .current_dir(dir.path())
        .args(["query", "code.rs", "1"])
        .assert()
...
/// `--json` that refusal must use the failure envelope on stdout, carrying
...
/// `confirm` advances the record of what the note body was last checked
/// against, so it must only run on a region the ladder actually identified.
/// An orphan has no region at all: confirming one would mark a note reviewed
/// against code nobody can point to. Exercised through the binary, because the
/// exit code is the part a script branches on and `Unconfirmable` has to reach
/// it as a usage error (2), not a generic failure (1).
#[test]
fn confirm_refuses_an_orphan_with_the_usage_exit_code() {
    let dir = tempfile::tempdir().expect("tempdir");
    ynotes()
        .current_dir(dir.path())
        .arg("init")
    ...
    let saved = ynotes()
        .current_dir(dir.path())
        .args(["save", "x.rs", "1:3", "-m", "why this is so", "--json"])
        .assert()
        .success();
    let saved: serde_json::Value =
        serde_json::from_slice(&saved.get_output().stdout).expect("valid JSON");
    let id = saved["data"]["id"].as_str().expect("an id").to_owned();

    // Replace the file with unrelated content: no content rung can locate the
    // region, so the note orphans.
    std::fs::write(&file, "fn something_else() {}\n").unwrap();
    ...
    let refused = ynotes()
        ...
        .assert()
        .code(2);
    let stderr = String::from_utf8_lossy(&refused.get_output().stderr).to_lowercase();
    assert!(
        stderr.contains("orphan"),
        "the refusal must say why, got: {stderr}"
    );

    // And it is a refusal, not a silent no-op: the note is untouched and still
    // present (invariant #4, an orphan is never dropped).
    let listed = ynotes()
        .current_dir(dir.path())
        .args(["list", "--json"])
        .assert()
    ...
    let listed: serde_json::Value =
        serde_json::from_slice(&listed.get_output().stdout).expect("valid JSON");
    assert_eq!(
        listed["data"]["notes"][0]["id"].as_str(),
        Some(id.as_str()),
        "the orphan keeps its id because nothing was confirmed"
    );
}
