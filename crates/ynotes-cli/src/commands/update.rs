//! `ynotes update` — replace an existing note's body in place.
//!
//! The note's target file, scope, and *currently resolved* line range are
//! reused; the selector bundle is re-captured against the current file so
//! the refreshed note anchors against the code as it is now. The new note
//! supersedes the old (its content-hash id is retired), and `created_at`
//! survives — it is the *same* note with a new body, not a new one.
