//! Read-only environment and store-health reporting.
//! Includes Git availability, malformed records, index drift, and merge guards.

use std::io::Write;

use serde::Serialize;

use ynotes::contract::{IndexHealthView, index_health_view};
...
struct HealthView {
    ...
    /// How far the index cache has drifted from the notes on disk. The shared
    /// contract view, so this block is byte-identical to the `index` object on
    /// `query`/`list`, one `status` vocabulary, defined once.
    ...
    fn from(h: ynotes::StoreHealth) -> Self {
        Self {
            ...
            malformed: h
                ...
                .collect(),
            index: index_health_view(h.index),
...
fn write_health_text(
    ...
    // Through the contract view, so the human line cannot drift from the word
    // `doctor --json` (and every other command) prints for the same state.
    let index_status = index_health_view(health.index).status;

    if health.index.status == ynotes::IndexStatus::Healthy
        && malformed == 0
        && recovered == 0
        && dangling == 0
        && guards == 0
    ...
    writeln!(
        out,
        "{:<10}{malformed} unreadable, index {index_status} ({recovered} recovered, {dangling} dangling), {guards} missing merge guard{}",
        "health",
        if guards == 1 { "" } else { "s" },
    )?;
    ...
    if malformed > 0 {
        // A record lives at `notes/<first 2 of id>/<remaining 62>.json`, so the
        // filename stem is only 62 of the 64 characters. Saying the path "ends
        // in the id" sent the reader to paste that stem, which passes prefix
        // validation, matches nothing, and returns "no match" with no hint that
        // the id was merely incomplete.
        writeln!(
            out,
            "  to remove one: its id is the directory name followed by the file \
             name without `.json`, run `ynotes delete <that 64-char id>`"
        )?;
    }
    for line in &health.gitattributes_missing {
        writeln!(out, "  missing gitattributes rule: {line}")?;
    }
    if health.index.status != ynotes::IndexStatus::Healthy || recovered > 0 || dangling > 0 {
        writeln!(
            out,
