...
use ynotes::contract::{NeighbourView, SaveData, scope_word_json};
use ynotes::{
...
fn run_inner(
    ...
    // Discover from cwd so an outside target is rejected against this store,
    // rather than selecting a different one. Initialisation precedes target
    // validation and can leave an empty store after a refused first save.
    let cwd = std::env::current_dir()?;
    let (store, store_created) =
        Store::discover_or_init_from(&cwd).map_err(CommandError::Engine)?;
    // Reject symlink escapes before relativisation so the diagnostic names
    // the resolved destination. Other outside paths use OutsideStore.
    ...
    let (created, superseded) = (outcome.created, outcome.superseded);
    ...
    if json {
        json_envelope::print_success(&SaveData {
            ...
            created,
            store_created,
            neighbours: NeighbourView::project(&outcome.neighbours),
        })?;
    } else {
        ...
        let corrected_clause = corrected_from
            ...
            .unwrap_or_default();
        // Match the JSON store_created signal and show the selected root.
        ...
        // Name what the file already said. Superseding only collapses an
        // identical location, so without this a restatement a few lines away
        // lands silently beside the original and nobody finds out until the
        // store is not worth reading.
        for n in &outcome.neighbours {
            writeln!(
                out,
                "  {} {} {}:{} {}",
                paint("also here", Colour::Dim),
                &n.id[..n.id.len().min(12)],
                n.bundle.position.range.start(),
                n.bundle.position.range.end(),
                ynotes::contract::body_excerpt(&n.body),
            )?;
        }
    }
    Ok(())
