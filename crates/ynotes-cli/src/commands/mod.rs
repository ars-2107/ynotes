...
pub(crate) fn dispatch(cli: &Cli) -> Result<(), CommandError> {
    match &cli.command {
        Command::Init { json } => init::run(*json),
        Command::Save {
            file,
            at,
            code,
        ...
        Command::Files { json } => files::run(*json),
        Command::Hook { event } => {
            match event {
                crate::cli::HookEvent::SessionStart => hook::session_start(),
            }
            Ok(())
        }
        ...
        Command::Confirm { id, json } => confirm::run(id, *json),
        ...
        Command::Skills { action } => skills::run(action),
