...
//!
//! Shape (`v: 1` of the agent contract):
//!
//! - success: `{"success": true, "v": 1, "data": <command payload>}`
//! - failure: `{"success": false, "v": 1, "error": "<message>", "type": "<kind>"}`
//!
...
pub(crate) fn print_error(message: &str, kind: ErrorKind) {
    ...
    let line = serde_json::to_string_pretty(&envelope).unwrap_or_else(|_| {
        // Bound to CONTRACT_VERSION so a version bump cannot strand a stale
        // literal in the one line no test ever renders.
        format!(
            r#"{{"success":false,"v":{CONTRACT_VERSION},"error":"failed to render JSON error envelope","type":"render"}}"#
        )
    });
