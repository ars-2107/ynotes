//! Parse query locations and render matching notes through [`super::render`].
//! Include orphaned notes for the target even when a line range is requested.

use std::path::Path;
use std::str::FromStr as _;

use ynotes::contract::{
    NoteView, QueryCountData, QueryData, QuerySpecView, count_view, index_health_view,
    malformed_view, note_view,
};
...
fn run_count_text(file: &Path, at: Option<&str>) -> Result<(), CommandError> {
    ...
    let result = ynotes::query(&store, file, spec)?;
    if let Some(advisory) = result.index.advisory() {
        eprintln!("ynotes: warning: {advisory}");
    }
    let count = count_view(result.matched.iter().chain(result.orphaned.iter()));
